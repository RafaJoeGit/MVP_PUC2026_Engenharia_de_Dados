# Análises Visuais — Inside Airbnb São Paulo

Cada gráfico abaixo foi construído em SQL sobre `fact_listing_dimensional`
(ou, no caso do Gráfico 01, sobre a tabela Gold `gold_neighbourhood_price`).
Os gráficos 02 a 09 aplicam o filtro `review_scores_rating IS NOT NULL`
de forma intencional — o recorte considera apenas anúncios com avaliação
registrada, o que pode gerar números ligeiramente diferentes dos
totais "brutos" documentados no `10_resultados.md` e no README
principal (que não aplicam esse filtro). Essa diferença de critério é
proposital e está anotada em cada gráfico onde se aplica.

## Gráfico 01 — Preço médio por bairro (tabela Gold oficial)
**Fonte:** `gold_neighbourhood_price`
**Pergunta relacionada:** 1
**Conclusão:** Reproduz, direto da tabela Gold oficial, o ranking de
...
**Conclusão:** Confirma a hierarquia de preço por tipo de acomodação já
...
**Conclusão:** Cruza três métricas por tipo de acomodação numa única
...
**Fonte:** `fact_listing_dimensional`, filtrado por `review_scores_rating IS NOT NULL`
**Pergunta relacionada:** 1
**Conclusão:** Identifica os bairros com maior concentração de oferta
de hospedagem, o que ajuda a interpretar o Gráfico 01/02: bairros caros
...
**Fonte:** `fact_listing_dimensional`, filtrado por `review_scores_rating IS NOT NULL`
**Pergunta relacionada:** 4
**Conclusão:** Aborda a relação avaliação/localização por um ângulo
que a correlação numérica da Pergunta 4 não captura: mesmo com
...
**Fonte:** `fact_listing_dimensional`, sem filtro de avaliação
**Pergunta relacionada:** 2
**Conclusão:** É o único gráfico de preço por tipo que usa exatamente
